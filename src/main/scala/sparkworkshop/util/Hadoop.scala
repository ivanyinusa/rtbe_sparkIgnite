...
object Hadoop {
  ...
  def apply(className: String, defaultOutpath: String, args: Array[String]): Unit = {
    ...
    val outpath = if (args2.length == 0) s"/user/$user/$defaultOutpath"
    ...
    val exitCode = s"/usr/local/project/sparkscalatutorial/scripts/hadoop.sh --class $className --out $outpath $argsString".!
