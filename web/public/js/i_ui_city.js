dojo.declare("i_ui_city", null, {
    ...
    init_map: function () {
        ...
        var gis_tip = d3.tip()
            ...
            .offset([6, 0])
            .html(function(d, i) {
                return "<b>City:</b>" + d.properties.city
                    + "<br>Revenue:loading....";
            });
        ...
        d3.json("map/usa/city/california.topo.json", function (error, d) {
            if (error) throw error;

            g.selectAll("path")
                //.data(d.features)
                .data(topojson.feature(d, d.objects.California.geo).features)
                .enter().append("path")
                .attr("d", path)
                .attr("class", "feature")
                .style("cursor", "hand")
            ...
            g.append("path")
                ...
                .datum(topojson.mesh(d, d.objects, function(a, b) { return a === b;}))
                .attr("class", "mesh")
                .attr("d", path);
        });
        function clicked(d) {
