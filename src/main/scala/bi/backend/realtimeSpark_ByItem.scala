...
import com.firebase.tubesock.{WebSocket, WebSocketEventHandler,WebSocketMessage,WebSocketException}
import bi.ignite.ItemAlarm

object realtimeSpark_ByItem extends App {
 ...
 */
  ...
  //kalfka settings
  //val topicMap = Map("EC_BuyBox" -> 1)
  val topicMap = Map("test" -> 2)
  val sparkConf = new SparkConf()
    .setMaster("local[2]")
    //.setAppName("EC_BuyBox_BI_Consumer")
  ...
  val kafkaConf = Map(
    ...
    //"group.id" -> "EC_BuyBox_BI_Consumer",
  ...
  val ic = new IgniteContext[String, ItemAlarm](sc, () => ic_conf())
  ...
  try {
    ...
    val conn = DriverManager.getConnection("jdbc:jtds:sqlserver://000.000.70.100:1433/NEStaging", "etl_sp", "1qazxsw2")
  ...
  def proccess_item(data: String): String = {
    ...
    try {
      ...
      if (BU == "USA") {
        ...
        try {
          //val m2 = scala.util.parsing.json.JSON.parseFull("test1").get.asInstanceOf[Map[String, Any]]
        ...
        if (item_of_usa != "" && (!item_store.containsKey(item_of_usa))) {
          //val m1 = scala.util.parsing.json.JSON.parseFull("test1").get.asInstanceOf[Map[String, Any]]
          ...
          item_store.put(item_of_usa, item)
          ...
          val item = item_store.get(item_of_usa)
          item.counter += 1
          item.dt = dt
          item.rank = rank
          item.q4s = q4s
          item.warehouse_type = warehouse_type
          item.runrate_type = runrate_type
          item.final_price = final_price
          item.timestamp = ar(4)
          item.winner_seller = winner_seller
          item.winner_price = winner_price
          item.newegg_price = newegg_price
          item.winner_score = winner_score
          item.newegg_score = newegg_score
          item.newegg_pcode = newegg_pcode
          item.newegg_eggpoint = newegg_eggpoint
          item.newegg_mir = newegg_mir
          item.newegg_aatc = newegg_aatc
          item.winner_vl = winner_vl
          item.winner_vm = winner_vm
          item.newegg_vl = newegg_vl
          item.newegg_vm = newegg_vm
          item.newegg_instock = newegg_instock
          item_store.put(item_of_usa, item)
          //         val itemJson=gson.toJson(itemInstance(item.country,item.item_of_mktpl,item.item_of_newegg,item.rank,formatter.format(item.dt.getTime()),item.item_desc,item.domain,item.grp_code,item.category,item.ctlg_code,item.subcategory,item.counter,item.q4s,item.warehouse_type,item.brand,item.runrate_type,item.neweggwh_only_avail,item.vf_only_avail,item.final_price))
          val itemJson = gson.toJson(item)
          ws.send("client:buybox:items:" + itemJson)
        }
        println(item_of_usa + " finished")
      } else {
