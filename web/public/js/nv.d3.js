        ...
        */
        ...
        ;
            ...
            nvtooltip.position = function(_) {
                ...
                window.status = "position.top=" + position.top;
                return nvtooltip;
            };
...
To enable in the chart:
  ...
  function dragEnd(d,i) {
    ...
    ;
        ...
        var showTooltip = function(e, offsetElement) {
        var today = offsetElement.childNodes[1].__data__[0].values[e.pointIndex].y;
        today = '$' + d3.format(',f')(today);
        var lastyear = offsetElement.childNodes[1].__data__[1].values[e.pointIndex].y;
        lastyear = '$' + d3.format(',f')(lastyear);
        var left = e.pos[0] + (offsetElement.offsetLeft || 0),
        top = e.pos[1] + (offsetElement.offsetTop || 0),
        x = xAxis.tickFormat()(lines.x()(e.point, e.pointIndex)),
        //y = (e.series.bar ? y1Axis : y2Axis).tickFormat()(lines.y()(e.point, e.pointIndex)),
        content = tooltip("Gross Product Sales", x, "<b style='color:red'>Today: " + today + "</b>&nbsp;&nbsp;&nbsp;&nbsp;1 Year Ago: " + lastyear, e, chart);
        //content = tooltip(e.series.key, x,y, e, chart);
        nv.tooltip.show([left, top], content, e.value < 0 ? 'n' : 's', null, offsetElement);
        }
    ;
    ...
    ;
        ...
        content = tooltip(e.series.key, x, y, e, chart);
          ...
          .text(function(d) { return d });
                    ...
                    gEnter.append('g').attr('class', 'nv-legendWrap');
                    gEnter.append('g').attr('class', 'nv-controlsWrap');
                    gEnter.append('g').attr('class', 'nv-averageLineWrap');
          ...
          .datum(data.filter(function(d) { return !d.disabled }))
            ...
            .tickSize(-availableWidth, 0);
              ...
              .call(yAxis);
                    ...
                    //*debug
                    if (averageValue > 0) {
                        //chart.yDomain([0, averageValue]);
                        var max = d3.max(data[0].values, function(d) {
                            return d.y;
                        });


                        if (averageValue > max) yAxis.scale(y).domain([0, averageValue]);

                        averageValue = d3.round(averageValue, 2);

                        var averageLineWrap = wrap.select('.nv-averageLineWrap') //.selectAll('.nv-averageLine')
                          .data(function(d) { return d });
                        averageLineWrap.enter().append('g').attr('class', 'nv-averageLine');
                        var avgLine = averageLineWrap.selectAll('.nv-averageLine').data(function(d) { return [d] });
                        avgLine.enter()
                     .append('line').attr('class', 'nv-averageLine')
                     .style('stroke-opacity', 0);
                     ...
                     .attr('x1', 0)
                     .attr('y1', function(d) { return y(averageValue) })
                     .attr('x2', function(d) { return (availableWidth) })
                     .attr('y2', function(d) { return y(averageValue) })
                     .style('stroke-opacity', 0.75)
                    ;

                        //          averageLineWrap.select('.avgLabel').remove();
                        //          averageLineWrap.append('text').attr('class', 'avgLabel')
                        //            .attr('x', (availableWidth-10)/2)
                        //            .attr('y', y(averageValue)-10)
                        //            .attr('dy', '.32em')
                        //            .attr('text-anchor', 'middle')
                        //            .text('Average = $'+averageValue.toString())
                    }
   ...
   'id', 'stacked', 'delay', 'barColor', 'groupSpacing');
            ...
            //*debug
            chart.averageValue = function(_) {
                if (!arguments.length) return averageValue;
                averageValue = _;
                return chart;
            };
            //============================================================
    ...
    ;
            ...
            /************************************
            * offset:
            *   'wiggle' (stream.stream)
            *   'zero' (stacked)
            *   'expand' (normalize to 100%)
            *   'silhouette' (simple centered)
            *
            * order:
            *   'inside-out' (stream.stream)
            *   'default' (input order)
            ************************************/
