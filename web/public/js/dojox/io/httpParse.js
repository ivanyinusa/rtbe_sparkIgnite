...
dojox.io.httpParse = function(/*String*/httpStream, /*String?*/topHeaders,/*Boolean?*/ partial){
	// summary:
	//		Parses an HTTP stream.stream for a message.
	// httpStream:
	//		HTTP stream.stream to parse
	// topHeaders:
	//		Extra header information to add to each HTTP request (kind of HTTP inheritance)
	// partial:
	//		A true value indicates that the stream.stream may not be finished, it may end arbitrarily in mid stream.stream.
	...
	do{
		...
		httpStream = httpStream.substring(content.length); // move along the stream.stream
